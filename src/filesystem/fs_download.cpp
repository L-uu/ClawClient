...
void fs_advance_download(void)
{
    ...
    if (current_download)
        fs_free_download_entry(current_download);
    ...
    while ((hashmap_entry = (fsc_pk3_hash_map_entry_t *)STACKPTRN(fsc_hashtable_next(&hti))))
    {
        ...
        if (fs_redownload_across_mods->integer && Q_stricmp(fsc_get_mod_dir((fsc_file_t *)pk3, &fs), entry->mod_dir))
        {
            ...
            if (different_moddir_match_out)
                *different_moddir_match_out = pk3;
            continue;
        }
        return qtrue;
    }
...
static qboolean fs_is_valid_download(download_entry_t *entry, unsigned int recheck_hash, qboolean curl_disconnected)
{
    ...
    if (entry_match_in_index(entry, &different_moddir_match))
    {
        ...
        {
            Com_Printf(
                "WARNING: Downloaded pk3 %s has unexpected hash which already exists in index."
                " Download not saved.\n",
                entry->local_name);
        }
        return qfalse;
    ...
    if (!recheck_hash)
    {
        ...
        {
            Com_Printf(
                "WARNING: Ignoring download %s because a download with the same hash has already been"
    ...
    if (different_moddir_match)
    {
        ...
        fs_file_to_buffer((fsc_file_t *)different_moddir_match, buffer, sizeof(buffer), qfalse, qtrue, qfalse, qfalse);
        Com_Printf(
            "WARNING: %s %s, even though the file already appears to exist at %s."
...
static download_entry_t *create_download_entry(const char *name, unsigned int hash)
{
    ...
    if (!fsc_get_leading_directory(name, temp_mod_dir, sizeof(temp_mod_dir), &temp_filename))
        return 0;
    if (!temp_filename)
        return 0;
    ...
    if (!Q_stricmp(mod_dir, fs_basegame->string))
        Q_strncpyz(mod_dir, fs_basegame->string, sizeof(mod_dir));
    ...
    entry->local_name =
        CopyString(va("%s/%s%s.pk3", mod_dir, fs_saveto_dlfolder->integer ? "downloads/" : "", filename));
...
void fs_advance_next_needed_download(qboolean curl_disconnected)
{
    ...
    {
        if (fs_is_valid_download(current_download, 0, curl_disconnected))
            break;
        fs_advance_download();
    }
}

qboolean fs_get_current_download_info(
    char **local_name_out, char **remote_name_out, qboolean *curl_already_attempted_out)
{
...
void fs_finalize_download(void)
{
    ...
    {
        const char *new_name = va("%s/%s%s.%08x.pk3", current_download->mod_dir,
    ...
    {
        Com_Printf(
            "ERROR: There was a problem moving downloaded pk3 %s from temporary file to target"
