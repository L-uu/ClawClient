...
typedef struct {
    ...
    // i.e. depth=0 suppresses any output, depth=1 allows "file" and "dir1/", depth=2 allows "dir1/file" and
    // "dir1/dir2/" Direct depth applies to files on disk (outside of pk3s), general applies to files in pk3s
...
static file_list_sort_key_t *generate_sort_key(const fsc_file_t *file, fsc_stack_t *stack, const filelist_work_t *flw)
{
    ...
    key =
        (file_list_sort_key_t *)FSC_STACK_RETRIEVE(stack, fsc_stack_allocate(stack, sizeof(*key) + stream.position), 0);
    ...
    while (1)
    {
        if (*pattern == '*' || initial_wildcard)
        {
            ...
            while (*string)
            {
                ...
                {
                    if (fs_pattern_match(string + 1, pattern + 1, qfalse))
                        return qtrue;
                }
                ++string;
            }
...
static void temp_file_set_insert(fs_hashtable_t *ht, const fsc_file_t *file, const char *path, qboolean directory,
    file_list_sort_key_t **sort_key_ptr, filelist_work_t *flw)
{
    ...
    if (ht->element_count >= MAX_FOUND_FILES)
        return;

    // Generate sort key if one was not already created for this file
    if (!*sort_key_ptr)
        *sort_key_ptr = generate_sort_key(file, &flw->temp_stack, flw);
...
static qboolean check_path_enabled(fsc_stream_t *stream, const filelist_work_t *flw)
{
    ...
    if (flw->extension)
    {
        if (!fs_pattern_match(stream->data, flw->extension, qtrue))
            return qfalse;
    }
    if (flw->filter)
    {
...
static qboolean check_file_enabled(const fsc_file_t *file, const filelist_work_t *flw)
{
    ...
    if (fs_file_disabled(file, disabled_checks))
        return qfalse;
    if ((flw->flags & FLISTFLAG_IGNORE_TAPAK0) && file->sourcetype == FSC_SOURCETYPE_PK3 &&
        fsc_get_base_file(file, &fs)->pk3_hash == 2430342401u)
        return qfalse;
    return qtrue;
}
...
static void cut_stream(const fsc_stream_t *source, fsc_stream_t *target, int start_pos, int end_pos)
{
    ...
    if (start_pos < end_pos)
        fsc_write_stream_data(target, source->data + start_pos, end_pos - start_pos);
...
{
    return temp_file_list_compare_element(
        *(const temp_file_set_entry_t **)element1, *(const temp_file_set_entry_t **)element2);
}
...
static char **temp_file_set_to_file_list(fs_hashtable_t *file_set, int *numfiles_out, filelist_work_t *flw)
{
    ...
    temp_file_set_entry_t **temp_list =
        (temp_file_set_entry_t **)Z_Malloc(sizeof(*temp_list) * file_set->element_count);
...
static fsc_directory_t *get_start_directory(const char *path)
{
    ...
    while ((directory_ptr = fsc_hashtable_next(&hti)))
    {
        ...
        if (path)
        {
            if (!Q_stricmp((const char *)STACKPTR(directory->qp_dir_ptr), path))
                break;
        }
        else
        {
            if (directory->qp_dir_ptr == 0)
                break;
        }
    }
...
static char **list_files(const char *path, int *numfiles_out, filelist_query_t *query)
{
    ...
    if (start_directory)
    {
        ...
        if (Q_stricmp(path_buffer1, path_buffer2))
        {
            if (fs_debug_filelist->integer)
                FS_DPrintf(
                    "NOTE: Restricting to direct files only due to OS-specific"
    ...
    else if (fs_debug_filelist->integer)
...
static int FS_GetModList(char *listbuf, int bufsize)
{
    ...
    for (i = 0; i < list.count; ++i)
    {
        ...
        if (!Q_stricmp(mod_name, fs_basegame->string))
            continue;
...
char **FS_FlagListFilteredFiles(
    const char *path, const char *extension, const char *filter, int *numfiles_out, int flags)
{
...
{
    // path, extension, and filter may be null
    int i;
    int flags = 0;
    int nFiles = 0;
    int nTotal = 0;
