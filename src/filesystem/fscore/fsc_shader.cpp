...
static int index_shader_file_data(
    fsc_filesystem_t *fs, fsc_stackptr_t source_file_ptr, char *data, fsc_errorhandler_t *eh)
{
    ...
    while (1)
    {
        ...
        new_shader->shader_name_ptr =
            fsc_string_repository_getstring(shader_name, 1, &fs->string_repository, &fs->general_stack);
