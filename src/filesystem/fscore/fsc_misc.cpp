...
{
    return fsc_stack_get_export_size(&fs->general_stack) + fsc_hashtable_get_export_size(&fs->string_repository) +
           fsc_hashtable_get_export_size(&fs->files) + fsc_hashtable_get_export_size(&fs->directories) +
           fsc_hashtable_get_export_size(&fs->shaders) + fsc_hashtable_get_export_size(&fs->crosshairs) +
...
int fsc_stack_export(fsc_stack_t *stack, fsc_stream_t *stream)
{
    ...
    if (fsc_write_stream_data(stream, &stack->buckets_position, 4))
        return 1;
...
int fsc_stack_import(fsc_stack_t *stack, fsc_stream_t *stream)
{
    ...
    if (fsc_read_stream_data(stream, &stack->buckets_position, 4))
        return 1;
    ...
    for (i = 0; i <= stack->buckets_position; ++i)
    {
        ...
        if (fsc_read_stream_data(stream, &stack->buckets[i]->position, 4))
            goto error;
...
unsigned int fsc_hashtable_get_export_size(fsc_hashtable_t *ht) { return 8 + ht->bucket_count * sizeof(*ht->buckets); }

int fsc_hashtable_export(fsc_hashtable_t *ht, fsc_stream_t *stream)
{
    // Returns 1 on error, 0 on success.
    if (fsc_write_stream_data(stream, &ht->bucket_count, 4))
        return 1;
    if (fsc_write_stream_data(stream, &ht->utilization, 4))
        return 1;
    if (fsc_write_stream_data(stream, ht->buckets, ht->bucket_count * sizeof(*ht->buckets)))
        return 1;
    return 0;
}
...
{
    // Returns 1 on error, 0 on success.
    if (fsc_read_stream_data(stream, &ht->bucket_count, 4))
        return 1;
    if (ht->bucket_count < 1 || ht->bucket_count > FSC_HASHTABLE_MAX_BUCKETS)
        return 1;
    if (fsc_read_stream_data(stream, &ht->utilization, 4))
        return 1;
...
fsc_stackptr_t fsc_string_repository_getentry(
    const char *input, int allocate, fsc_hashtable_t *string_repository, fsc_stack_t *stack)
{
...
fsc_stackptr_t fsc_string_repository_getstring(
    const char *input, int allocate, fsc_hashtable_t *string_repository, fsc_stack_t *stack)
{
    // Return type is char *
    fsc_stackptr_t entry = fsc_string_repository_getentry(input, allocate, string_repository, stack);
    if (entry)
        return entry + sizeof(stringrepository_entry_t);
    return 0;
}
...
const char *fsc_get_qpath_conversion_table(void)
{
    ...
    if (qpath_conversion_table_initialized)
        return qpath_conversion_table;

    // Default to underscore
    for (i = 0; i < 256; ++i)
        qpath_conversion_table[i] = '_';
