...
#define EOCD_SHORT(offset) \
    fsc_endian_convert_short(*(unsigned short *)(buffer + sizeof(buffer) - eocd_position + offset))
#define EOCD_INT(offset) fsc_endian_convert_int(*(unsigned int *)(buffer + sizeof(buffer) - eocd_position + offset))
    ...
    if (EOCD_INT(4) != 0)
        return 1;  // "this file's disk number" and "disk number containing central directory" should both be 0
    if (EOCD_SHORT(8) != EOCD_SHORT(8))
        return 1;  // "cd entries on this disk" and "cd entries total" should be equal
    ...
    // Determine zip offset from error in reported cd position - all file offsets need to be adjusted by this value
    {
        ...
        if (cd_position_reported > cd_position)
            return 1;  // cd_position is already the maximum valid position
    ...
    {
        unsigned int buffer_file_position =
            file_length - buffer_read_size;  // Position in file where buffered data begins
...
static int get_pk3_central_directory_path(
    void *os_path, central_directory_t *output, fsc_file_direct_t *source_file, fsc_errorhandler_t *eh)
{
    // Returns 1 on error, 0 on success
    void *fp = 0;
    unsigned int length;
    ...
    unsigned int uncompressed_size, short compression_method, fsc_errorhandler_t *eh)
{
    ...
    file->f.qp_dir_ptr =
        qp_dir ? fsc_string_repository_getstring(qp_dir, 1, &fs->string_repository, &fs->general_stack) : 0;
    file->f.qp_name_ptr = fsc_string_repository_getstring(qp_name, 1, &fs->string_repository, &fs->general_stack);
    file->f.qp_ext_ptr =
        qp_ext ? fsc_string_repository_getstring(qp_ext, 1, &fs->string_repository, &fs->general_stack) : 0;
    ...
    void (*receive_hash_data)(void *context, char *data, int size), void *receive_hash_data_context)
{
    ...
    if (get_pk3_central_directory_path(os_path, &cd, sourcefile, eh))
        return;
...
#define CD_ENTRY_INT_LE(offset) (*(unsigned int *)(cd.data + entry_position + offset))
        ...
        filename_length = (int)CD_ENTRY_SHORT(28);
        {
            ...
            {
                fsc_report_error(eh, FSC_ERROR_PK3FILE, "invalid file cd entry position 2", sourcefile);
                goto freemem;
            }
        }
        ...
        {
            fsc_report_error(eh, FSC_ERROR_PK3FILE, "invalid file local entry position 1", sourcefile);
            goto freemem;
        }
        if (header_position + compressed_size > FSC_MAX_PK3_SIZE)
        {
            fsc_report_error(eh, FSC_ERROR_PK3FILE, "invalid file local entry position 2", sourcefile);
            goto freemem;
        }
            ...
            !(!uncompressed_size && *(cd.data + entry_position + 46 + filename_length - 1) == '/'))
        {
            ...
            register_file_from_pk3(fs, cd.data + entry_position + 46, filename_length, sourcefile_ptr, header_position,
                compressed_size, CD_ENTRY_INT(24), CD_ENTRY_SHORT(10), eh);
        }

        ++entry_counter;
        entry_position += entry_length;
        if (entry_counter >= cd.entry_count)
            break;
    }  // Abort if that was the last file
...
void *fsc_pk3_handle_open(
    const fsc_file_frompk3_t *file, int input_buffer_size, const fsc_filesystem_t *fs, fsc_errorhandler_t *eh)
{
...
static int fsc_pk3_extract_data(
    const fsc_file_t *file, char *buffer, const fsc_filesystem_t *fs, fsc_errorhandler_t *eh)
{
    ...
    if (!handle)
        return 1;
    if (fsc_pk3_handle_read(handle, buffer, file->filesize) != file->filesize)
        result = 1;
...
fsc_sourcetype_t pk3_sourcetype = {
    FSC_SOURCETYPE_PK3, fsc_pk3_is_file_active, fsc_pk3_get_mod_dir, fsc_pk3_extract_data};
