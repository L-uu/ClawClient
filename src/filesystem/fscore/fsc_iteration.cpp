...
static int get_parent_qp_dir(const char *qp_dir, char *target)
{
    ...
    if (length > FSC_MAX_QPATH - 1)
        length = FSC_MAX_QPATH - 1;
    fsc_memcpy(target, qp_dir, length);
    target[length] = 0;
    return 1;
}

static fsc_stackptr_t get_directory(
    const char *qp_dir, fsc_hashtable_t *directories, fsc_hashtable_t *string_repository, fsc_stack_t *stack)
{
...
void fsc_iteration_register_file(
    fsc_stackptr_t file_ptr, fsc_hashtable_t *directories, fsc_hashtable_t *string_repository, fsc_stack_t *stack)
{
    fsc_file_t *file = (fsc_file_t *)STACKPTR_LCL(file_ptr);

    // Get directory
    fsc_stackptr_t directory_ptr =
        get_directory((const char *)STACKPTRN_LCL(file->qp_dir_ptr), directories, string_repository, stack);
