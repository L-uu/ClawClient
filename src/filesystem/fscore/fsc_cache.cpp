...
unsigned int integer_hash(unsigned int value) { return value; }
...
fsc_stackptr_t convert_string(fsc_stackptr_t source_string, export_work_t *xw)
{
    ...
    return fsc_string_repository_getstring(
        (const char *)STACKPTR_SRC(source_string), 1, &xw->export_string_repository, &xw->export_stack);
}
...
fsc_stackptr_t convert_file(fsc_stackptr_t source_file_ptr, export_work_t *xw)
{
    ...
    if (source_file->sourcetype == FSC_SOURCETYPE_DIRECT)
        ...
        fsc_fatal_error("convert_file called on unsupported sourcetype");
    ...
    fsc_hashtable_insert(export_file_ptr,
        fsc_string_hash(
            (const char *)STACKPTR_SRC(source_file->qp_name_ptr), (const char *)STACKPTRN_SRC(source_file->qp_dir_ptr)),
        &xw->export_files);
    fsc_iteration_register_file(
        export_file_ptr, &xw->export_directories, &xw->export_string_repository, &xw->export_stack);
...
void fscache_build_filesystem(export_work_t *xw)
{
    ...
    for (i = 0; i < xw->source_fs->files.bucket_count; ++i)
    {
        ...
        while ((source_file_ptr = fsc_hashtable_next(&hti)))
        {
            ...
            if (!fsc_is_file_enabled(source_file, xw->source_fs))
                continue;
            if (source_file->sourcetype == FSC_SOURCETYPE_PK3 ||
    ...
    for (i = 0; i < xw->source_fs->shaders.bucket_count; ++i)
    {
        ...
        while ((source_shader_ptr = fsc_hashtable_next(&hti)))
        {
            ...
            if (source_file->sourcetype != FSC_SOURCETYPE_DIRECT && source_file->sourcetype != FSC_SOURCETYPE_PK3)
                continue;
            if (!is_shader_enabled(xw->source_fs, source_shader))
                continue;
...
void fscache_build_crosshairs(export_work_t *xw)
{
    ...
    for (i = 0; i < xw->source_fs->crosshairs.bucket_count; ++i)
    {
        ...
        while ((source_crosshair_ptr = fsc_hashtable_next(&hti)))
        {
            ...
            if (source_file->sourcetype != FSC_SOURCETYPE_DIRECT && source_file->sourcetype != FSC_SOURCETYPE_PK3)
                continue;
            if (!is_crosshair_enabled(xw->source_fs, source_crosshair))
                continue;
...
void fscache_build_pk3_hash_lookup(export_work_t *xw)
{
    ...
    for (i = 0; i < xw->source_fs->pk3_hash_lookup.bucket_count; ++i)
    {
        ...
        while ((source_entry_ptr = fsc_hashtable_next(&hti)))
        {
            ...
            if (!fsc_is_file_enabled((fsc_file_t *)source_pk3_file, xw->source_fs))
                continue;
...
static int fsc_cache_export_data(fsc_filesystem_t *source_fs, fsc_stream_t *stream)
{
    ...
    stream->size =
        fsc_stack_get_export_size(&xw.export_stack) + fsc_hashtable_get_export_size(&xw.export_string_repository) +
        fsc_hashtable_get_export_size(&xw.export_files) + fsc_hashtable_get_export_size(&xw.export_directories) +
        fsc_hashtable_get_export_size(&xw.export_shaders) + fsc_hashtable_get_export_size(&xw.export_crosshairs) +
    ...
    if (!error_state)
        if (fsc_stack_export(&xw.export_stack, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_string_repository, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_files, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_directories, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_shaders, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_crosshairs, stream))
            error_state = 1;
    if (!error_state)
        if (fsc_hashtable_export(&xw.export_pk3_hash_lookup, stream))
            error_state = 1;
...
static int fsc_cache_import_data(fsc_stream_t *stream, fsc_filesystem_t *target_fs)
{
    ...
    if (fsc_stack_import(&target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->string_repository, &target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->files, &target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->directories, &target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->shaders, &target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->crosshairs, &target_fs->general_stack, stream))
        goto error;
    if (fsc_hashtable_import(&target_fs->pk3_hash_lookup, &target_fs->general_stack, stream))
        goto error;
